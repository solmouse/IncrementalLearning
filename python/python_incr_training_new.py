...
CKPT_BEFORE = os.path.join(WORK_DIR, "ckpt_before")
CKPT_AFTER  = os.path.join(WORK_DIR, "ckpt_after")
...
acc_A_before = evaluate_with_ckpt(TFLITE_PATH, CKPT_BEFORE + '.npy', test_ds)
acc_A_after  = evaluate_with_ckpt(TFLITE_PATH, CKPT_AFTER  + '.npy',  test_ds)
...
acc_B_after = evaluate_with_ckpt(TFLITE_PATH, CKPT_AFTER + '.npy', test_ds_B)
...
if bool(res["approve"]):
    print("증분학습 결과 반영")
    final_ckpt = CKPT_AFTER + ".npy"
else:
    print("증분학습 결과 반려")
    final_ckpt = CKPT_BEFORE + ".npy"
