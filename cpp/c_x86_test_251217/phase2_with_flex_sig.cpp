...
struct FeatureExtractor {
    ...
    bool load(const char* f) {
        ...
        std::cout << "[BACKBONE] Initialize complete" << std::endl;

        // Now allocate tensors for infer
    ...
    void extract(const float* img, float* feat, int bs) {
        ...
        for (int n = 0; n < bs; ++n) {
            // Copy single image (batch size = 1 in signature)
            memcpy(
                input->data.f,
...
int main(int argc, char** argv) {
    ...
    // ============================================================================
    // 0. Initial Validation (Before Training)
    // ============================================================================
    std::cout << "\n=== Initial Validation (Epoch 0 - Pre-training) ===" << std::endl;
    {
        ...
        std::cout << "  Initial Overall Accuracy: " << std::fixed << std::setprecision(4)
