...
echo '=== 5. Build x86_64 Shared Libraries ==='
echo "Building libtensorflowlite.so for x86_64..."
bazel build -c opt \
  --config=monolithic \
...
echo "Building libtensorflowlite_flex.so for x86_64..."
bazel build -c opt \
  --config=monolithic \
  --define=tflite_with_select_tf_ops=true \
  --verbose_failures \
...
echo '=== 7. Zip SDK ==='
cd /tmp
zip -r tflite_sdk_x86.zip sdk/
ls -lh tflite_sdk_x86.zip

echo '=== ✓ Build Complete ==='
echo "x86_64 SDK successfully built: /tmp/tflite_sdk_x86.zip"
echo "Ready for deployment to x86_64 Linux systems"
