...
BASE_DIR = r"D:\2025-1\lge\LGE_IncrmentalLearning\pi\python_pi_test_251129"
...
HEAD_LR = 8e-4
BASE_LR = HEAD_LR * 0.1
FINE_TUNE_FRACTION = 0.05        # 베이스 뒤쪽 25% conv 레이어만 미세조정
...
# 기존 코드 끝에 추가
print("\n=== Trainable Variables Check ===")
print("Head trainable vars:", len(m.head.trainable_variables))
for v in m.head.trainable_variables:
    print(f"  {v.name}: {v.shape}")

print("\nBase trainable vars:", len([v for v in m.base.trainable_variables if v.trainable]))
for v in m.base.trainable_variables:
    if v.trainable:
        print(f"  {v.name}: {v.shape}")

# save 크기 확인
saved = m.save()['weights']
print(f"\nSaved weights size: {saved.shape[0]}")